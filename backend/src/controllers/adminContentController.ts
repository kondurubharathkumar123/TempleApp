...
export async function getAdminGallery(_req: Request, res: Response) {
  try {
    const result = await pool.query(`SELECT id, title, description, image_url, category, is_active FROM gallery ORDER BY id DESC`);
    res.json({ success: true, data: result.rows });
  } catch (error) {
...
export async function createGalleryItem(req: Request, res: Response) {
  try {
    const { title, description, image_url, category, is_active = true } = req.body;
    if (!title?.trim() || !image_url?.trim()) return res.status(400).json({ success: false, message: 'Title and image URL are required' });
    const result = await pool.query(`
      INSERT INTO gallery (title, description, image_url, category, is_active)
      VALUES ($1,$2,$3,$4,$5) RETURNING id, title, description, image_url, category, is_active
    `, [title.trim(), description?.trim() || null, image_url.trim(), category?.trim() || null, Boolean(is_active)]);
    res.status(201).json({ success: true, message: 'Gallery item created successfully', data: result.rows[0] });
  } catch (error) {
...
export async function updateGalleryItem(req: Request, res: Response) {
  try {
    ...
    const result = await pool.query(`
      UPDATE gallery SET
        title=COALESCE($1,title), description=COALESCE($2,description), image_url=COALESCE($3,image_url),
        category=COALESCE($4,category), is_active=COALESCE($5,is_active)
      WHERE id=$6
      RETURNING id, title, description, image_url, category, is_active
    `, [title?.trim() || null, description?.trim() || null, image_url?.trim() || null, category?.trim() || null, typeof is_active === 'boolean' ? is_active : null, id]);
    if (!result.rows.length) return res.status(404).json({ success: false, message: 'Gallery item not found' });
    res.json({ success: true, message: 'Gallery item updated successfully', data: result.rows[0] });
  } catch (error) {
...
export async function deactivateGalleryItem(req: Request, res: Response) {
  try {
    const result = await pool.query(`UPDATE gallery SET is_active=FALSE WHERE id=$1 RETURNING id, title, is_active`, [req.params.id]);
    if (!result.rows.length) return res.status(404).json({ success: false, message: 'Gallery item not found' });
    res.json({ success: true, message: 'Gallery item deactivated successfully', data: result.rows[0] });
  } catch (error) {
    console.error('Deactivate gallery item error:', error);
    res.status(500).json({ success: false, message: 'Failed to deactivate gallery item' });
  }
}
